...
export default function WeatherComponent(): JSX.Element {
  ...
  const [precipitationDays, setPrecipitationDays] = useState(
    Array(7).fill(null)
  );
  ...
  const fetchDataForecasts = async (
    data: WeatherData,
    sunDown: string,
    sunUp: string
  ) => {
    ...
    let chartData1 = createChartData(hourly, (item) => {
      const forecastDateTime = new Date(item.dt * 1000);
      const forecastDay = forecastDateTime.toLocaleDateString("en-US", {
        timeZone: data.timezone,
      });
    ...
    const temperaturesDailyMax = forecastsDaily.map(
      (forecast: WeatherForecast) => Math.floor(forecast.temp.max)
    );
    const temperaturesDailyMin = forecastsDaily.map(
      (forecast: WeatherForecast) => Math.floor(forecast.temp.min)
    );
    const precipitationDaily = forecastsDaily.map(
      (forecast: WeatherForecast) => +(forecast.pop * 100).toFixed(0)
    );
    const weatherIdsDaily = forecastsDaily.map(
      (forecast: WeatherForecast) => forecast.weather[0].id
    );
    const days = ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"];
    const dates = forecastsDaily.map(
      (forecast: WeatherForecast) => new Date(forecast.dt * 1000)
    );
  ...
  const fetchDataCurrent = async (
    ...
    if (
      ...
      console.error(
        "No weather array found or weather array is empty:",
        data.current.weather
      );
    ...
    const localDate = new Date(
      utcDate.getTime() + timezoneOffsetHours * 60 * 60 * 1000
    );
    ...
    const offsetMilliseconds =
      (date.getTimezoneOffset() + timezoneOffsetMinutes) * 60 * 1000;
    ...
    if (alerts && Array.isArray(alerts)) {
      if (
        alerts.some(
          (alert: WeatherAlert) =>
            alert.event && alert.event.includes("thunder")
        )
      )
        setThunderMessage("THUNDERSTORM WARNING");
      if (
        alerts.some(
          (alert: WeatherAlert) =>
            alert.event &&
            (alert.event.includes("high-temperature") ||
              alert.event.includes("heat"))
        )
      )
        setHeatMessage("HIGH TEMPERATURE WARNING");
      if (
        alerts.some(
          (alert: WeatherAlert) =>
            alert.event && alert.event.includes("flooding")
        )
      )
        setFloodMessage("FLOOD WARNING");
      if (
        alerts.some(
          (alert: WeatherAlert) =>
            alert.event && alert.event.includes("snow-ice")
        )
      )
        setIceMessage("ICE WARNING");
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "City not found or API error";
          ...
          try {
            ...
            const errorMessage =
              error instanceof Error
                ? error.message
                : "A problem occurred during geolocation...";
            setError(errorMessage);
          } finally {
            setLoading(false);
          }
  ...
  const handleUnity = () => {
    if (temperature?.endsWith("C")) {
      ...
      const temperatureInFahrenheit = (temperatureInCelsius * 1.8 + 32).toFixed(
        1
      );
      ...
      setTempMinDays(
        tempMinDays.map(
          (temp) => `${(temp.slice(0, -2) * 1.8 + 32).toFixed(0)}°F`
        )
      );
      setTempMaxDays(
        tempMaxDays.map(
      ...
      const feelsLikeInCelsius = ((feelsLikeInFahrenheit - 32) / 1.8).toFixed(
        0
      );
      ...
      setTempMinDays(
        tempMinDays.map(
          (temp) => `${((temp.slice(0, -2) - 32) / 1.8).toFixed(0)}°C`
        )
      );
      setTempMaxDays(
        tempMaxDays.map(
      ...
      <header className="mb-4">
        ...
        {showComponents && (
          <div className="flex justify-between items-center">
            <Text className="text-gray-600 text-base">{time} | ARDABIL</Text>
            <Button
              type="text"
              onClick={handleUnity}
              shape="circle"
            ...
            {(thunderMessage || heatMessage || floodMessage || iceMessage) && (
              <Card className="mb-4 bg-amber-50 border border-amber-200">
                <Space direction="vertical" className="w-full">
                  {thunderMessage && (
                    <Alert message={thunderMessage} type="warning" showIcon />
                  )}
                  {heatMessage && (
                    <Alert message={heatMessage} type="warning" showIcon />
                  )}
                  {floodMessage && (
                    <Alert message={floodMessage} type="warning" showIcon />
                  )}
                  {iceMessage && (
                    <Alert message={iceMessage} type="warning" showIcon />
                  )}
                </Space>
              </Card>
            )}
              ...
              <Row gutter={24} align="middle">
                ...
                <Col xs={24} sm={16}>
                  <div className="text-center sm:text-left">
                    <Title level={1} className="mb-0 text-blue-600">
                      {temperature}
                    </Title>
                    <Title level={5} className="capitalize mt-0 text-gray-600">
                      {description}
                    </Title>
